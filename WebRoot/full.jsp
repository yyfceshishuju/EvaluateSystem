      ...
      $(function(){
        var treeData = [{
			...
			children : [  {
				text : "讲师教评项管理",
				attributes : {
					url : "<%=request.getContextPath()%>/bgTemplate/evaluateItemManagePage.jsp",
				}
			},{
				text : "讲师教评模板管理",
				attributes : {
					url : "<%=request.getContextPath()%>/bgTemplate/templateManagePage.jsp",
				}
			},{
				text : "讲师教评管理",
				attributes : {
            		  url:"<%=request.getContextPath()%>/bk/evaluateList.jsp",
				}
			}]	
		},{
          text:"助教教评管理",
          ...
          children:[{
            text:"助教教评项管理",
            attributes:{
              url : "<%=request.getContextPath()%>/bgTemplate/evaluateItemManageZJPage.jsp",
            }
          },{
            text:"助教教评模板管理",
            attributes:{
              url : "<%=request.getContextPath()%>/bgTemplate/templateManageZJPage.jsp",
            }
          },{
            text:"助教教评管理",
            attributes:{
              url:"<%=request.getContextPath()%>/bk/evaluateListZj.jsp",
			}
		} ]			
		},{
          text:"测评信息汇总",
          ...
          children:[{
            text:"上传用户数据",
            attributes:{
              url : "<%=request.getContextPath()%>/bk/importUsers.jsp",
            }
          },{
            text:"上传讲师测评数据",
            attributes:{
              url : "<%=request.getContextPath()%>/bk/importTeacher.jsp",
            }
          },{
            text:"上传助教测评数据",
            attributes:{
              url : "<%=request.getContextPath()%>/bk/importAssistant.jsp",
            }
          }]
		},{
          text:"发送邮件",
          ...
          children:[{
            text:"测评结果邮件发送",
            attributes:{
              url : "<%=request.getContextPath()%>/bk/sendEmail.jsp",
            }
          }]
		}		
		];
