   ...
   <script type="text/javascript">
	...
	$('#ff').hide();
		...
		$('#tt').datagrid({
			...
			striped : true,
			collapsible : true,
			url : '<%=request.getContextPath()%>/evaluateZj/evaluateZj_showEvaluateDetails.action?id=${param.id}&tag=a', 
			loadMsg : '数据装载中......',
			...
			columns : [ [ {
				...
				formatter:function(value,rowDate){
					var str1 = "<a href='<%=request.getContextPath()%>/evaluateZj/evaluateZj_showEvaluateItemDetail.action?id="+rowDate.id+"&templateId="+${param.templateId}+"&evaluateId="+evaluateId;
              		...
              		$.messager.confirm('确认', '确定导出?', function(r){
						if (r){
	                		 ...
	                		 var rows = $(".pagination-page-list :selected").text();
	                 		 location.href="<%=request.getContextPath()%>/exportData/exportEvaluateDetailZJById.action?zjEvaluateId="+evaluateId; 
						}
					});
                }
              },"-",{
               ...
               handler : function() {
              		$.messager.confirm('确认', '确定导出?', function(r){
						if (r){
							 page = $(".pagination-num").val();
	                		 var rows = $(".pagination-page-list :selected").text();
	                 		 location.href="<%=request.getContextPath()%>/exportData/exportEvaluateDetailZJ.action?evaluateId="+evaluateId; 
						}	
					});
                }
              },"-",{
